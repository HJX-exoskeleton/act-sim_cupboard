...
from utils import sample_box_pose, sample_insertion_pose, sample_box_cupboard_pose
...
class CupboardEETask(BimanualViperXEETask):
    ...
    def initialize_episode(self, physics):
        ...
        box_pose, target_box_pose = sample_box_cupboard_pose()
        id2index = lambda j_id: 16 + (j_id - 16) * 7  # first 16 is robot qpos, 7 is pose dim # hacky
    ...
    def get_reward(self, physics):
        ...
        # touch_left_gripper = ("handle_box", "vx300s_left/10_left_gripper_finger") in all_contact_pairs
        ...
        touch_target = ("green_box", "target_box") in all_contact_pairs

        reward = 0
        if touch_right_gripper:
            reward = 1
        # if touch_left_gripper:  # lifted
        #     reward = 2
        if touch_right_gripper and not touch_table:  # attempted transfer
            reward = 2
        if touch_target and not touch_table:  # successful transfer
            reward = 4
        return reward
